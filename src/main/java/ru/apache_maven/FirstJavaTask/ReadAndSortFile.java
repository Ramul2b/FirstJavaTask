...
/**
 * Reads a strings from a text file input.txt, sort and write them into the file
 * output.txt.
 * 
 * @version 1.8.
...
public class ReadAndSortFile {
	...
	/**
	 * Reads a string of characters in the text file input.txt and writes them into
	 * an array.
	 * The file specified in the run configuration.
	 * 
	 * @param inputTextFile - text file input.txt from which the symbols are read.
	 * @return textArray - array of sorted rows. For sorting used static method
	 *         {@link sort}.
	 * @throws IOException when an IO error occurs. Triggered when not right given
	 *         the way of the files.
	 */
	public static ArrayList<String> writeInArrayAndSort(String inputTextFile) {
		...
		try {
			...
			/**
			 * Class BufferedReader reads characters from the input character stream
			 * and buffering to effectively read them.
			 * Class FileReader used to read the character (text) files.
			 */
			BufferedReader readTextFile = new BufferedReader(new FileReader(
					inputTextFile));
			...
			/**
			 * As long as a character string not null, they are added in the array
			 * textArray.
			 * This means that an array of lines from input.txt.do recorded as long
			 * as have something recording.
			 */
	...
	/**
	 * Records sorted lines to the file output.txt. The file specified in the run
	 * configuration.
	 * 
	 * @param textArray - an array that contains a sorted string.
	 * @param outputTextFile - a text file in which records sorted lines.
	 * @throws IOException if an error occurs IO. Triggered when not right given the
	 *         way the files.
	 */
	public static void writeInOutputFile(ArrayList<String> textArray,
		...
		try {
			BufferedWriter writeTextFile = new BufferedWriter(new FileWriter(
					outputTextFile));
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out
					.println("Index of args[] is wrong. Check path-name of files.");
		}
